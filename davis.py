    ...
    while True :
        for event in pygame.event.get():
            ...
            if (event.type == pygame.QUIT) :
                ...
                main_menu()
            elif(event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and videoButton.rect.collidepoint(pygame.mouse.get_pos())):
                main_menu()
            elif (event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and fullscreenButton.rect.collidepoint(pygame.mouse.get_pos())):
                gameDisplay = pygame.display.set_mode((width,height), pygame.FULLSCREEN)
