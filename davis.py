...
def options_menu():
    ...
    volumeDisplay = Button(str(funcs.roundup(math.trunc(current_volume * 100))), config.white, config.SPOOKY_SMALL_FONT, (w/2,h/2),gameDisplay)
    ...
    Button("1920 x 1080", config.white, config.SPOOKY_SMALL_FONT, (w / 2, h / 2 - 100), gameDisplay)]
    ...
    while True :
        ...
        for event in pygame.event.get():
            ...
            if (event.type == pygame.QUIT) :
                ...
                pygame.display.update()
                main_menu()
            elif (event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and buttons[5].text == "1920 x 1080" and
